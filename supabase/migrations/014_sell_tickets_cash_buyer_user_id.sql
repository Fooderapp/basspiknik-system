-- Add optional p_buyer_user_id to sell_tickets_cash so tickets sold at the
-- POS to a registered user (identified by wallet QR scan) are linked to their
-- account and appear in their ticket list.
...
begin
  ...
  -- Create order — link to registered buyer's account when available
  insert into orders
