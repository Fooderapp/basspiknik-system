-- Migration 018: Door ticket — sold at entrance, auto-admitted on creation
--
-- ticket_types.is_door_ticket = true means:
--   tickets of this type are created with status = 'USED' and a check_in
--   record is inserted immediately (person paid at the door and walked in).

alter table ticket_types
  add column if not exists is_door_ticket boolean not null default false;

-- Update sell_tickets_cash: auto-admit when ticket type is marked is_door_ticket
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    for v_i in 1..(v_item->>'quantity')::int loop
      -- Door tickets: immediately USED + check-in record
      insert into tickets
